...
const FinishedJob = ({ job }: { job: FinishedJob }) => {
    ...
    return (
        <div className="job job-finished">
            ...
            <a className="job-url" href={job.tag}>{job.tag}</a>
...
const App = () => {
    ...
    const handleClick = async () => {
        ...
        try {
            return void await browser.tabs.executeScript(undefined, {file: '/dist/content.js'});
        }
        catch(e) {
    ...
    return (
        <div id="popup-content">
            {jobs.size > 0 &&
            Array.from(itiriri(jobs.values()).map((job) =>
