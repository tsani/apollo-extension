...
export class JobManager {
    ...
    constructor(
        ...
        checkPeriod = CHECK_DOWNLOAD_PERIOD_MS,
    ) {
    ...
    async addJob(job: RunningJob): Promise<void> {
        this.jobs.set(job.tag, {
    ...
    async deleteJob(tag: string): Promise<void> {
    ...
    private async updateJobs(): Promise<void> {
        ...
        if (isActive) this.scheduleUpdateJobs();
    }
        ...
        await matchJob<Promise<Job>>({
            ...
            onRunning: async (job) =>
                matchJobStatus<Promise<Job>>({
                    ...
                    onRunning: async (status) =>
                        injectRunning({ ...job, ...status }),
                })(await this.apolloClient.checkJob(job.queryUrl)),
        })(job);
}

const noop = async <T>(_: T): Promise<void> => { return; };
